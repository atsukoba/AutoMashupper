    ...
    def test_adjust_tempo_basic(self, sample_audio_long):
        ...
        try:
            ...
            if "rubberband" in str(e).lower():
                pytest.skip(
                    f"adjust_tempo test skipped - rubberband-cli not available: {e}"
                )
            else:
...
class TestMixSongs:
    ...
    def test_mix_songs_basic(self, sample_audio_long):
        ...
        try:
            ...
            if "rubberband" in str(e).lower():
                pytest.skip(
                    f"mix_songs test skipped - rubberband-cli not available: {e}"
                )
            else:
    ...
    @pytest.mark.parametrize(
        "beat_offset,pitch_shift", [(0, 0), (4, 2), (8, -2), (2, 1), (6, -1)]
    )
    def test_mix_songs_different_parameters(
        self, sample_audio_medium, beat_offset, pitch_shift
    ):
        ...
        try:
            ...
            result = mix_songs(
                main_song, cand_song, beat_offset=beat_offset, pitch_shift=pitch_shift
            )
...
class TestSelfTempoEstimation:
    ...
    def test_self_tempo_estimation_basic(self, sample_audio_long):
        ...
        try:
            ...
            if isinstance(result, (tuple, list)):
                ...
                tempo_value = (
                    result[0]
                    if isinstance(result[0], (int, float, np.number))
                    else result
                )
            else:
                tempo_value = result
    ...
    def test_self_tempo_estimation_different_sample_rates(
        self, sample_audio_medium, sr
    ):
...
class TestUtilityPerformance:
    ...
    def test_tempo_adjustment_performance(self, sample_audio_long):
        ...
        try:
            ...
            if "rubberband" in str(e).lower():
                pytest.skip(
                    f"Tempo adjustment test skipped - rubberband-cli not available: {e}"
                )
            else:
